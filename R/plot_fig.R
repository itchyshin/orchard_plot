...
p1 <- orchaRd::orchard_plot(english_MR, mod="ManipType", es_type = "Hedges' d") + 
...
p2 <- orchard_plot(senior_MR, mod = "ManipType", es_type = "log Coefficient of Variation (ln CVR)", alpha = 0.8) + 
	guides(fill = "none", colour = "none") +  
...
p3 <- orchard_plot(eklof_MR, mod = "Grazer.type", es_type = "log Response Ratio (lnRR)") + 
	guides(fill = "none", colour = "none") +  
  	theme(legend.position= c(0.05, 0.99), legend.justification = c(0,1), legend.key.size = unit(1, "mm")) +
  	theme(legend.direction="horizontal", legend.title = element_text(size =8),legend.text = element_text(size = 8)) + 
scale_x_continuous(expand = c(0.2,0.2))

p4 <- orchard_plot(lim_MR, mod = "Phylum", es_type = "Zr", alpha = 0.60) + 
  	guides(fill = "none", colour = "none") +  
  	theme(legend.position= c(0.05, 0.99), legend.justification = c(0,1), legend.key.size = unit(1, "mm")) +
  	theme(legend.direction="horizontal", legend.title = element_text(size =8),legend.text = element_text(size = 8)) + 
  	scale_x_continuous(expand = c(0.2,0.2))
