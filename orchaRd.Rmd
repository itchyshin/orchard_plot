...
In addition to orchard plots, we have another plotting function `caterpillars` . Furthermore, the pacakge `orchaRd` comes with two functions useful for mulitlevel meta-analysis and meta-regression (i.e., `rma.mv`): 1) `i2_ml` to calculate the 'total' I^2^ along with I^2^ for each level (*sensu* Nakagawa & Santos 2012; see also higgins....XXXX; Senior et al. 2016) and 2) `r2_ml` to calcuate marginal and conditional R^2^ () for mixed models (*sensu* Nakagawa & Schielzeth 2013; see also ....). 
...
english_MA<-metafor::rma.mv(yi=yi, V=vi, random=list(~1|EffectID), data=english)
summary(english_MA)
...
In Figure \ref{fig:Figure1}, we simply add in the metafor model and it will create a default orchard plot. Alternatively, we could also add in the table of results. Some people may wish to present the heterogeneity index, I^2^ (also, we change the colours of the 'trunk and furit' by adding a few arguments). <---comments ---->
...
```{r Figure6, echo = TRUE, eval = TRUE, messages = FALSE, warnings = FALSE, fig.width = 6, fig.height = 8, fig.cap = "\\label{fig:Figure6}Orchard plot of the the strength of correlation between maternal and offspring size within-species"}
  ...
  #Plot the meta-regression model
	p7 <- orchard_plot(lim_MR, mod = "Phylum", xlab = "Correlation coefficient", 
...
TODO - add ggswarm plot ref and related - NS x 2 papers
