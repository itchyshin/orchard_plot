...
Above, we simply add in the metafor model and it will create a default orchard plot. Alternatively, we could also add in the table of results and also we take advantage of element wise addition so unsers can adjust the look of their figures quite easily. In this instance, we change colours of the overall estimate and effect sizes:

```{r echo = TRUE, eval = TRUE, fig.width = 6, fig.height = 3, fig.cap = "Orchard plot of diet qualities impact on standardised mean difference but instead of using the metafor model, using the model results table and grey colour"}
