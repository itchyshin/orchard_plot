...
# Lets fit a meta-regression - I will do Article non-independence. The phylogenetic model found phylogenetic effects, however, instead we could fit Phylum as a fixed effect and explore them with an Orchard Plot
  ...
  # From the above models, we can also calculate the R2, the proportion of variance explained by Phylum, which will be useful down the line.  - SHIN, PLEASE CHECK THIS IS CORRECT, I assume we don't  include sampling variance???? Also, I did one, but then found a post by Wolfgang suggesting an "alternative", but they give different answers so worth you checking this..(https://stackoverflow.com/questions/22356450/getting-r-squared-from-a-mixed-effects-multilevel-model-in-metafor)
...
p1 <- orchard_plot(lim_MR_int, mod = "Phylum", xlab = "Correlation coefficient", 
    alpha = 0.5, transfm = "tanh", angle = 45, N = lim$N, cb = FALSE) + 
  
  annotate(geom="text", x=1, y=0.50, label=paste0("I^{2}[study] == ", I2_study),
    ...
    annotate(geom="text", x=0.5, y=7.4, label=paste0("R^{2} == ", R2),
              color="black", parse = TRUE, size = 6)
```
