...
english <- metafor::escalc(measure = "CVR", n1i = NStartControl, sd1i = SD_C, m1i = MeanC, 
    n2i = NStartExpt, sd2i = SD_E, m2i = MeanE, data = english, append = TRUE, var.names = c("lnCVR.yi", 
        "lnCVR.vi"))
...
senior_MR <- metafor::rma.mv(yi = lnCVR.yi, V = lnCVR.vi, mods = ~ManipType - 1, 
    random = list(~1 | EffectID), data = english)
summary(senior_MR)
#> 
#> Multivariate Meta-Analysis Model (k = 77; method: REML)
#> 
...
#> Error in escalc(measure = "ROM", n1i = N_control, sd1i = SD_control, m1i = mean_control, : could not find function "escalc"
...
#> Error in rma.mv(yi = yi, V = vi, mods = ~Grazer.type - 1, random = list(~1 | : could not find function "rma.mv"
```
...
p3 <- orchard_plot(eklof_MR, mod = "Grazer.type", xlab = "log(Response ratio) (lnRR)", 
    transfm = "none")
#> Error in class(object) %in% c("rma.mv", "rma"): object 'eklof_MR' not found
...
# Here we will fit an intercept only meta-analytic model to ascertain the overall
# correlation between maternal and offspring size.
lim_MR_int <- metafor::rma.mv(yi = yi, V = vi, random = list(~1 | Article, ~1 | Datapoint), 
    data = lim)
summary(lim_MR_int)
#> 
...
#> 
#>    logLik   Deviance        AIC        BIC       AICc 
#> -100.6261   201.2523   207.2523   218.8771   207.3205   
#> 
#> Variance Components:
#> 
#>             estim    sqrt  nlvls  fixed     factor 
#> sigma^2.1  0.0429  0.2071    220     no    Article 
#> sigma^2.2  0.0299  0.1730    357     no  Datapoint 
#> 
#> Test for Heterogeneity:
#> Q(df = 356) = 1980.6060, p-val < .0001
#> 
#> Model Results:
#> 
#> estimate      se     zval    pval   ci.lb   ci.ub 
#>   0.3761  0.0202  18.5736  <.0001  0.3364  0.4158  *** 
#> 
#> ---
#> Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

# Lets estimate heterogenity for the model using I2 - SHIN, PLEASE CHECK THIS IS
# CORRECT calculate weights and total sampling variance
wi <- 1/lim$vi
...
# From the above models, we can also calculate the R2, the proportion of variance
# explained by Phylum, which will be useful down the line.  - SHIN, PLEASE CHECK
# THIS IS CORRECT, I assume we don't include sampling variance???? Also, I did
# one, but then found a post by Wolfgang suggesting an 'alternative', but they
# give different answers so worth you checking
# this..(https://stackoverflow.com/questions/22356450/getting-r-squared-from-a-mixed-effects-multilevel-model-in-metafor)

R2 <- round(var(predict(lim_MR)$pred)/(sum(lim_MR$sigma2) + var(predict(lim_MR)$pred)), 
    digits = 3)  # Alternative approach
...
orchard_plot(lim_MR_int, mod = "Phylum", xlab = "Correlation coefficient", alpha = 0.5, 
...
![\label{fig:Figure6}Orchard plot of the the strength of correlation between maternal and offspring size within-species](figure/Figure6-2.png)
...
![\label{fig:Figure7}Orchard plot of the the strength of correlation between maternal and offspring size within-species](figure/Figure7-2.png)
