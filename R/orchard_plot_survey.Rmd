...
We found "classical forest plot" in (need double-checking with full text):     
...
myVaggr4 <- plotVenn(list(taxon_aggr = papers_taxon, study_aggr=papers_study, individual_aggr=papers_individual, all_papers=all), nCycles = 6000) #, outFile = "myVaggr.svg"
...
#with hand-picked "real forest" papaer added:
...
myVaggr4 <- plotVenn(nVennObj = myVaggr4) #re-run simulation
showSVG(nVennObj = myVaggr4, opacity = 0.1, labelRegions = F, fontScale = 3, setColors = c('yellow', 'teal', 'red', 'green', 'black'), borderWidth = 3) # hiding region labels, big fonts, thick borders and custom colors
```
...
**Answer 5.**  
Final list of papers presenting "original/classical" forest plots (relevant figure file):     
...
```
 ...
 Excluded papers with forest-like plots (relevant figure file), with reasons for exclusion:
  1. ft037 (ft037_figS4-3.png) - no overall mean effect sieze shown in teh same plot panel    
  2. ft084 (ft084_figS1.png) - no overall mean effect sieze shown in teh same plot panel      
  3. ft109 (ft109_figS9.png, ft109_figS12.png, ft109_figS13.png, ft109_figS14.png) - no overall mean effect sieze shown in teh same plot panel      
  4. ft181 (ft181_fig2.png) - no overall mean effect sieze shown in teh same plot panel      
