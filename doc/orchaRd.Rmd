---
title: "orchaRd: An R package for drawing an orchard plot from a meta-regression with a categorical moderator"
...
english_MR_int <- rma.mv(yi = SMD, V = vSMD, random = list( ~ 1 | StudyNo, ~ 1 | EffectID), data = english)
...
res1 <- mod_results(english_MR_int, mod="Int")
print(res1)
...
orchard_plot(english_MR_int,
  mod = "Int", xlab = "Standardised mean difference",
  transfm = "none"
) 
```

In \ref{fig:Figure1}, we simply add in the metafor model and it will create a default orchard plot. Alternatively, we could also add in the table of results, and try to change the colour of the 'trunk' (estimate) and 'fruit' (effect sizes) from the defult (orange) to grey by just adding `ggplot` functions.
...
Figures \ref{fig:Figure1} & \ref{fig:Figure2} above show that overall estimate from a random-effects meta-analysis of 77 effect sizes is centered on zero, with a 95% CI that spans the line of no-effect. The prediction intervals clearly demonstrate the high level of heterogeneity, with effects size less than -0.5 and greater than 0.5 predicted to be observed. 

In a subsequent publication, Senior et al. (2017) analysed this dataset for effects of dietary-restriction on among-individual variation in the age at death using the log coefficient of variation ratio. A major prediction in both English & Uller (2016) and Senior et al. (2017) was that the type pf manipulation, whether the study manipulated quality of food versus the quantity of food would be important. As such, we can fit a meta-regression model to test whether the moderator "Manipulation Type" impacts our results on the mean and variance. Note that we need a meta-regression model with a categorical varaible for `orchard_plot` (i.e. the intercept less model). 
...
# we fit a meta-regression (contrast)
senior_MR0 <- rma.mv(
...
# fit a meta-regression without the intercept
eklof_MR <- rma.mv(
  yi = lnRR, V = vlnRR, mods = ~ Grazer.type - 1, random = list(~ 1 | ExptID, ~ 1 | Datapoint),
  data = eklof
)
...
The other thing we can change is the type of scaling we wish to use. Let's say we are interested in scaling the effect size by the total sample size of the study we use a vector of N, sample size (bottom of Figure \ref{fig:Figure5}):
...
Now that we have Figure \ref{fig:Figure6} we can do some small changes to make it pretty. Currently, the cb argument is "FALSE", we can change this to "TRUE" to use colour blind friendly colours. Additionally, because we are using `ggplot2` we can add element to the figure to make it look pretty. 
