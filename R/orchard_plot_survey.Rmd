...
output:
  ...
  pdf_document:
    toc: yes
subtitle:  "The Orchard Plot: Cultivating a Forest Plot for Use in Ecology, Evolution and Beyond"
...
From the full texts and SI of 102 meta-analytic papers, we took screenshots of any figures that look like a forest plot (presenting individual or aggregated effect sizes). The screenshots included figure captions, which usually provided information needed to tell what is presented in the plots (e.g., type of point estimates or intervals presented). This resulted in 295 .png files with the figure screenshots. The files were named with the fulltext_ID (as in the list of collected 102 meta-analytic papers), followed by an underscore and the figure number (e.g., ft001_fig1.png or ft001_figS1.png, for the main text or supplementary info, respectively). The files are stored in a private folder FOREST_PLOTS on OSF).   

## Literature survey - STAGE1      

The STAGE 1 survey was designed to answer the following questions:   
 1.	How many papers (percentage from 102 surveyed) present some form of forest plots? (at this stage we could not tell which of these are “classic/original” forest plots, the more detailed questions below with more detailed coding were used to assess this).   
 2.	How many papers (percentage from 102 surveyed) present prediction interval in forest plots? (basing on the coding).     
 ...
 5. What is the frequency of "classic/original" forest plots?     
 
 We had the following predictions for the above questions:    
  1. We expected that it is commonly used.   
  2. We expected mean with CI to be the most common way of presenting point estimates and interval estimates, respectively.    
  3. We expected “dot” to be most popular, as this is the “classic/original” forest plot style; we expected dot scaling to be likely underutilized.   
  4. We expected there should be many studies with taxon-level aggregation, but "other aggregated" could dominate.  
  5. We expected few occurrences of “classic/original” forest plots (showing all individual effect sizes used in an meta-analysis alongside their mean, in a "dot" graphical style).    

### Data extraction
We designed a data extraction form to gather information on the features of the collected forest plots (Google Form screenshot available in "Supplementary File 1.png"). Briefly, we assessed only figure panels that have forest plot-like style graphs (ignore funnel plots, histograms, etc.). We based our classification on the information present in the plot itself and plot caption (except if CI are shown with point estimates, we assumed the latter to be the means).     

For the "data aggregation level" we coded whether individual (non-aggregated) effect sizes were presented, or they were aggregated at the study, taxon or other levels. Plot graphical style was coded as dot (a shape, usually circles, squares or diamonds, used to represent point estimate, with whiskers representing interval estimate, as in the “classic” forest plot), box (vertical or horizontal line representing point estimate, box edges represent dispersion), bar (edge of a horizontal or vertical bar  representing point estimate; we excluded histograms of distributions of individual values), mixed (combination of any of the above, e.g., “classic/real” forest plot with individual data points alongside, as in the proposed orchard plot) (Figure 1). 

![an image caption Source: graphical styles of forest plots](Graphical_style_conceptual_diagram_v1.png)
*Figure 1. Main graphical styles of forest-like plots.*      
...
### 1. How many papers present some form of forest plots?       
   
**Answer 1.**	   
Forest plots are commonly used in ecology and evolution - `r length(unique(dat$paperID))` papers (`r round(length(unique(dat$paperID))/102*100, 2)`%, out of 102 surveyed papers ) had at least one forest plot-like figure.    


### 2.	How many papers present prediction interval in forest plots?  
...
**Answer 2.**	
Only one (`r papers_PI`; Fig.3) paper presented prediction interval in a forest plot (`r round(length(papers_PI)/102*100, 2)`%). Most commonly, as expected, confidence or credible intervals were used - `r length(papers_CI)` papers (`r round(length(papers_CI)/102*100, 2)`%). Occasionally, also SD or SE were presented - `r length(papers_SDorSE)` papers (`r round(length(papers_SDorSE)/102*100, 2)`%).   

Reference of the paper with PI:    
...
*Figure 2. Types of intervals used in surveyed papers.*     
...
*Figure 3. Graphical styles used in surveyed papers.*     
*Legend description (numbers of papers with at least one plot with given feature):*   
...
**Answer 3b.**     
Here we looked at whether dot shapes are scaled to represent their properties (sample sizes, precision, etc.) Only `r length(papers_scaling)` papers (`r round(length(papers_scaling)/102*100, 2)` %; `r round(length(papers_scaling)/length(papers_dot)*100, 2)` % of papers with dot-style plots), used point estimate scaling in their forest plots.   
...
*Figure 4. Presence of dot scaling in surveyed papers.*      
*Legend description (numbers of papers with at least one plot with given feature):*   
...
**Answer 4.**    
As expected, not many studies contained plots that looked like potential “classic” forest plots (aggregated at study level) - `r length(papers_study)` papers (`r round(length(papers_study)/102*100, 2)`%). Notably, this number does not represent the occurrence of "classic" forest plots, as they need to fulfill also other criteria, and this will be evaluated in one of the subsequent questions. There were more studies with taxon-level data aggregation forest plots - `r length(papers_taxon)` papers (`r round(length(papers_taxon)/102*100, 2)`%). And, also as expected, "other aggregated" forest plots dominated - `r length(papers_other)` papers (`r round(length(papers_other)/102*100, 2)`%). There were very few that showed individual effect sizes in forest plots - `r length(papers_individual)` papers (`r round(length(papers_individual)/102*100, 2)`%).   
...
*Figure 5. Aggregation levels used in surveyed papers.*     
*Legend description (numbers of papers with at least one plot with given feature):*   
...
forest = potential papers with "classic/real" forest-plots (to be manually re-evaluated) (N = `r length(potential_forest)`)              
all_other = all other papers with forest(-like) plots      

Table of papers with potential classic forest plots (to be re-evaluated to see if they are actually can be considered "classic" forest plots):     
```{r table of papers with potential classic forest plots, eval = TRUE}
# making a scrollable table
...
### 5. What is the frequency of "classic/original" forest plots?    
We re-evaluated the subset of 16 papers with potential "original/classic" forest plots.    
We used the following definition of "classic/original" forest plot:    
...
**Answer 5.**  
Out of 16 papers re-assessed, 12 contained  "original/classic" forest plots (see table below for the references).   

```{r table of papers with actual classic forest plots, eval = TRUE}
...
The remaining 4 re-assessed papers were excluded due to not showing the overall mean effect size in the same plot panel.          
...
*Figure 6. aggregation levels in surveyed papers relatively to papers with classic forest plots.*     
*Legend description (numbers of papers with at least one plot with given feature):*   
tax = papers with effect sizes aggregated to taxon level in the forest(-like) plots (N = `r length(papers_taxon)`)       
stu = papers with effect sizes aggregated to study level in the forest(-like) plots (N = `r length(papers_study)`)         
ind = papers with individual effect sizes presented in the forest(-like) plots (N = `r length(papers_individual)`)     
forest = actual papers with "classic/real" forest-plots (manually re-evaluated) (N = `r length(real_forest)`)      
all_other = all other papers with forest(-like) plots         
...
To answer this question we re-evaluated a subset of `r length(papers_taxon)` papers which used some sort of taxonomic grouping for aggregating effect sizes. We then created a list of papers containing at least one plot with species-level aggregation of effect sizes. IDs of re-evaluated papers: `r sort(papers_taxon)`     

**Answer 6.**    
Final list of 7 papers presenting "species" forest-like plots:     
...
myVaggr6 <- plotVenn(list(all_other = all, classic = real_forest, species = papers_species), nCycles = 6000) #, outFile = "myVaggr.svg"
...
*Figure 7. Species-level aggregation in surveyed papers.*    
*Legend description (numbers of papers with at least one plot with given feature):*   
species = papers with effect sizes aggregated to species level in the forest(-like) plots (N = `r length(papers_species)`)       
classic = actual papers with "classic/real" forest-plots (manually re-evaluated) (N = `r length(real_forest)`)    
all_other = all other papers with forest(-like) plots    
...
The remaining  papers were excluded due to all relevant figures not showing individual effect sizes as dots overlayed onto aggregated effect sizes.     
...
Additional questions to be answered in STAGE2 coding:    
 8. Do forest-like plots present an overall meta-analytic mean?     
 9. Do forest-like plots present point estimates calculated separately for subsets of data (as in medical “summary forest plot”, where a meta-analytic model is run for each subset of effect sizes and meta-analytic mean for the subset is presented, so called “subgroup analysis” in medicine) or these point estimates are based on meta-regressions models (i.e. model of all data with a moderator(s)).    

Predictions: we expected to find a mixture of forest-like plots (based on meta-regressions, without overall mean) and summary forest plots (based on subgroup analyses, often with overall mean) in ecological and evolutionary meta-analyses. Summary forest plots could be considered a special case of the forest-like plots.    
...
classic_IDs <- c("ft065_fig1", "ft065_fig2.png", "ft065_fig3.png", "ft078_figS1", "ft082_figS4", "ft098_fig2", "ft112_fig2", "112_fig3", "112_figS3","112_figS4", "112_figS5", "ft131_fig1",  "ft148_figS2", "148_figS4", "148_figS7", "148_figS10", "148_figS18", "148_figS19", "ft151_fig1", "ft167_fig2", "167_figSB", "ft173_fig2", "ft175_figS4", "175_figS5", "175_figS6", "175_figS7", "ft177_fig2")  
...
#exclude "classic" forest plots and "individual" forest plots
toasses <- setdiff(dat$fileID, classic_IDs) 
...
### 10. What is the frequency of caterpillar plots with individual effect sizes plotted with CI?         

To answer this question, we considered a definition of a caterpillar plot as:     
 **a)** a classic forest plot (non-aggregated dot-type with CI) without overall mean (Kossmeier et al. 2020. Charting the landscape of graphical displays for meta-analysis and systematic reviews: a comprehensive review, taxonomy, and feature analysis BMC Med Res Methodol. 2020; 20: 26).    
 **b)** individual ES are ordered by ES magnitude, with CI (Spiegelhalter D.J. 2005. Funnel plots for comparing institutional performance. Statistics in Medicine,24(8), 1185-1202); they can have or not have labels or overall means displayed.       
For these two definitions we created a list of papers containing at least one plot matching the given definition (a or b) of a caterpillar plot.    
     
**Answer 10.**     
Papers presenting caterpillar-like plots:     
 **a)** 5 papers with classic-like forest plots but not showing the overall mean effect size in the same plot panel: ft037 (Fig. S4-3), ft084 (FigS1-S2), ft109(FigS9, FigS12-S14), ft148 (Fig10), ft181 (Fig2)     
 **b)** 4 papers with forest plots (effect sizes with CI) ordered by ES magnitude: ft098 (Fig2), ft167 (Fig2, AppendixB), ft177 (Fig2), ft181 (Fig2)     
