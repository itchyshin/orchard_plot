---
title: "orchaRd: An R package for drawing 'orchard' plots (and 'caterpillar' plots) from meta-analyses and meta-regressions with categorical moderators"
...
# Examples of orchard plots along with caterpillar plots
...
```{r Figure1, echo = TRUE, eval = TRUE, fig.width = 6, fig.height = 4, fig.cap = "Orchard plot of the impact caloric restriction using standardised mean difference"}
...
```{r Figure2, echo = TRUE, eval = TRUE, fig.width = 6, fig.height = 4, fig.cap = "Orchard plot of the impact of caloric restriction using standardised mean difference but instead of using the metafor model, using the model results table", messages = FALSE, warnings = FALSE}
...
```{r Figure3, echo = TRUE, eval = TRUE, fig.width = 6, fig.height = 6, fig.cap = "Catterpilar plot of the impact of caloric restriction using standardised mean difference", messages = FALSE, warnings = FALSE}

# a caterpillar plot (not a caterpillars plot)
caterpillars(model_results, mod="Int", xlab = "Standardised mean difference") 
```

It is now interesting to compare how the same data set can be visualised differently by looking at Figure \@ref(fig:Figure2) and Figure \@ref(fig:Figure3). The caterpillar plot can show CIs for each effect size. This is depicted in the orchard plot by the size of fruit when using precision.
...
```{r Figure4, fig.width = 6, fig.height = 8, fig.cap = "Orchard plot of diet qualities impact on SMD (Top panel) and log coefficient of variation (Bottom panel)"}
...
Let's draw comparable caterpillar plots to compare these two results again. 

```{r Figure5, echo = TRUE, eval = TRUE, fig.width = 6, fig.height = 8, fig.cap = "Caterpillar plot of diet qualities impact on the standardised mean difference (SMD) (Top Panel) and log coefficient of variation (lnCVR) (Bottom panel)", messages = FALSE, warnings = FALSE}
...
```{r Figure6, echo = TRUE, eval = TRUE, fig.width = 6, fig.height = 8, fig.cap = "Orchard plots of the effects of predation on benthic invertebrate communities compared using the log response ratio. Top panel is the default plot and bottom panel a plot containing changes in label axes"}
...
Again, we can also draw the caterpillar-plot counterparts for this dataset (\@ref(fig:Figure8)):
...
# Lets fit a meta-regression - We will do Article non-independence. The phylogenetic model found phylogenetic effects, however, instead we could fit Phylum as a fixed effect and explore them with an Orchard Plot
  lim_MR<-metafor::rma.mv(yi=yi, V=vi, mods=~Phylum-1, random=list(~1|Article, ~1|Datapoint), data=lim)
  summary(lim_MR)
  
```

Now we can plot a default orchard plot, scaling each effect size by N. Also, because we are using Zr, we can use transfm = "tanh" and it will do the conversions for us back to the scale of raw correlation coefficients:
...
Finally, we make a comparable caterpillar plot.
